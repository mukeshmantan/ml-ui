...
$(document).ready(function() {
    ...
    if (!urlDefault)
        urlDefault = "http://52.158.239.241:9000/classify";
	var imagePathDefault = "/Users/liule/att/lei-connector-cooler/test";
    demo_init({
        ...
        protoList: [["/static/model.image.proto", true], ["/static/model.tag.proto", false]]
    });
});
 ...
 */
function genCoolerTable (data, div) {
	$(div).empty();
	var classTable = $('<table />').append($('<tr />')
        ...
        classTable.append($('<tr />')
            .append($('<td />').append(data['type']))
            .append($('<td />').append(data['score']))
            );
