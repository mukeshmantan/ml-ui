...
function demo_init(objSetting) {
    ...
    $("#imageUrl").change(function() {
        $(document.body).data('hdparams')['imagePath'] = $(this).val();
    }).val($(document.body).data('hdparams')['imagePath'])
...
function switchImage(imgSrc, isVideo) {
	...
	console.log("===============selectedImagePath:" + canvas.toDataURL())
    if (!isVideo) {
        var img = new Image();
...
function doPostImage(srcCanvas, dstDiv, dstImg, imgPlaceholder) {
    ...
    console.log("===============hd.protoKeys:" + hd.protoKeys)
...
function doPostPayload(sendPayload, domHeaders, dstDiv, dstImg, imgPlaceholder)
{
    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR) {
			...
			console.log("============response score:" + data['score']);
            ...
            //hd.imageIsWaiting = false;
            //return returnState;
			var returnState = genCoolerTable(data, dstDiv);
           	return returnState;
